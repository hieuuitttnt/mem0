...
class MemoryGraph:
    ...
    def delete(self, data, filters):
        ...
        Args:
            ...
            filters (dict): Contains user_id (required), agent_id (optional)
        ...
        try:
            ...
            for item in entities_with_relations:
                ...
                # Build filter properties for queries
                if filters.get("agent_id"):
                    match_props = "user_id: $user_id, agent_id: $agent_id"
                    params = {
                        "source": source,
                ...
                # Decrement mentions for relationship and connected nodes
                # Delete relationship if mentions reaches 0
                cypher = f"""
                MATCH (s:Entity {{name: $source, {match_props}}})
                -[r:{relationship}]->
                (d:Entity {{name: $destination, {match_props}}})
            ...
            if filters.get("agent_id"):
                ...
                """
                cleanup_params = {"user_id": filters["user_id"], "agent_id": filters["agent_id"]}
            else:
                cleanup_cypher = """
                MATCH (n:Entity {user_id: $user_id})
                WHERE n.mentions <= 0 
